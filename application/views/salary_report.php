                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            ...
                            <li class="active">Laporan Salary</li>
                        </ol>
                    </div>
                </div>
            </div>
                        ...
                        <div style="margin-left: 5px;">
                            <select id="selectedUser" name="user" class="form-control form-control-sm select2 user">
                                ...
                                <?php if ($this->input->get('user')) : ?>
                                    <option value="<?= $user->id ?>" selected><?= $user->name . ' (' . $user->telephone . ')' ?></option>
                                <?php endif ?>
                            </select>
                        </div>
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $(".btn-print").on("click", function() {
                        var start = jQuery("input[name=start_date]").val();
                        var end = jQuery("input[name=end_date]").val();
                        var user = jQuery("select[name=user]").val();

                        if (start && end) {
                            var url = "<?= base_url("report/salary_pdf"); ?>/" + start + "/" + end;
                        } else {
                            var url = "<?= base_url("report/salary_pdf"); ?>";
                        }

                        let paramUser = user !== null ? `?user=${user}` : ''

                        location.href = url + paramUser
                    })
